...
- ✅ **Burger menu scrollbar hidden** — canonical `.menu-overlay__content` was showing a default browser scrollbar across the dark seamless overlay. Added `scrollbar-width: none` (Firefox), `::-webkit-scrollbar { display: none }` (Chromium/Safari), and `-ms-overflow-style: none` (IE/old Edge). Scrolling stays functional. Re-vendored to TAT.
- ✅ **Public-asset comment hygiene** — stripped date stamps, sign-off attributions, internal review-tool references (Moirai/Ratio), cross-site origin notes, sibling-incident leaks, and debugging anecdotes from public deployed CSS / JS / HTML. 17 canonical components + the `marbl-core` bundle + TAT-specific `build.mjs` / landing.css / kh-content.css. Kept: API contracts, WCAG references, license headers, intentional public attribution. Re-runnable script lives at `marbl-codes/scripts/comment-hygiene.mjs` (gitignored).
- ✅ **marbl.codes legacy pages aligned to canonical 120% accent** — about.html, index.html, ecosystem.html, marbl-home.css local overrides bumped from 1.1em / 1.15em to 120%. Committed `--no-verify` per Richard's authorisation (pre-commit lint blocks on pre-existing token-debt unrelated to this change).
